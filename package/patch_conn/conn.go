...
// patch 暴露统计流量hook，实现http长连接情况下，能够实时更新流量
// "github.com/SianHH/frp-package/pkg/util/net"
...
type StatsConn struct {
	...
	interval   time.Duration // hook 调用间隔
	lastTime   time.Time
	deltaRead  int64
	deltaWrite int64
}

func WrapStatsConn(conn net.Conn, statsFunc func(total, totalWrite int64), closeFunc func(), interval time.Duration) *StatsConn {
	return &StatsConn{
		Conn:      conn,
		statsFunc: statsFunc,
		interval:  interval,
...
// flush 用于间隔触发 hook
func (statsConn *StatsConn) flush(force bool) {
	if statsConn.statsFunc == nil {
		return
	}
	now := time.Now()
	if force || statsConn.interval <= 0 || now.Sub(statsConn.lastTime) >= statsConn.interval {
		if statsConn.deltaRead > 0 || statsConn.deltaWrite > 0 {
			statsConn.statsFunc(statsConn.deltaRead, statsConn.deltaWrite)
			statsConn.deltaRead = 0
			statsConn.deltaWrite = 0
			statsConn.lastTime = now
		}
	}
}
...
func (statsConn *StatsConn) Read(p []byte) (n int, err error) {
	...
	statsConn.deltaRead += int64(n)
	statsConn.flush(false)
	return
}
...
func (statsConn *StatsConn) Close() (err error) {
	...
	if old != 1 {
		...
		statsConn.flush(true)
		if statsConn.closeFunc != nil {
			statsConn.closeFunc()
		}
		//if statsConn.statsFunc != nil {
